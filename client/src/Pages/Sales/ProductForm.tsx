...
const ProductForm = ({ index, close, productItem }: {
    index?: number;
    close: () => void;
    productItem?: Product;
}) => {
    ...
    const onSubmit = (data: {
        ...
        setLoading(true);
        if (product) {
            dispatch(createSale({ ...data, product: product, userId: user._id } as Sale)).unwrap()
                .then(() => {
                    dispatch(addSale({ ...data, product: product, _id: '', userId: user._id } as Sale));
                    setLoading(false);
                })
        }
        reset();
