...
const Dashboard = () => {
    ...
    const [salesByDate, setSalesByDate] = useState<{ date: string; totalSales: number }[]>([]);
    useEffect(() => {
        const salesByDate = sales.reduce((acc: { [key: string]: { date: string; totalSales: number } }, sale) => {
        ...
        const sortedSalesByDate = Object.values(salesByDate).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
